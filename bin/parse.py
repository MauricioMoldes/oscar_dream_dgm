...
def insert_specimen(specimen_id, person_id, procedure_occurrence_id, specimen_concept_id, specimen_date, anatomic_site, disease_status, conn):
    """
    ...
    :param conn: Active PostgreSQL database connection
    """
    query = "INSERT INTO SPECIMEN (specimen_id, person_id, procedure_occurrence_id, specimen_concept_id, specimen_date, anatomic_site, disease_status) VALUES(%s, %s, %s, %s, %s, %s, %s);"
    
    try:
        with conn.cursor() as cur:
            cur.execute(query, (specimen_id, person_id, procedure_occurrence_id, specimen_concept_id, specimen_date, anatomic_site, disease_status))           
            conn.commit()
...
def insert_procedure_occurrence(procedure_occurrence_id, person_id, procedure_concept_id, procedure_date, 
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(query, (procedure_occurrence_id, person_id, procedure_concept_id, 
                                procedure_date, procedure_type_concept_id))
            conn.commit()
            print("Procedure occurrence inserted successfully")
    except Exception as e:
        conn.rollback()
...
def extract_genomic_test():
    """
    Techincal specification of genomic test performed on sote 
    """
    genomic_test_id=1
    care_site_id=1
